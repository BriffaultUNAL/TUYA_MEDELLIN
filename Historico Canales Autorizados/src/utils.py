...
def webscraping(import_username: str, import_password: str, *_):
    ...
    try:
        ...
        asyncio.run(enviar_mensaje(f'Tuya canales autorizados'))
        ...
        driver.get(
            """https://172.226.124.46/vsmart/index.php?rtr=informes&ctr=InformesControlador&acc=&nom_programa=informe_historico_canales""")

        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.CLASS_NAME, "select2-container"))).click()

        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, "//*[@id='select2-drop']/ul/li[5]/div"))).click()
    ...
    try:
        with engine_61.get_connect() as conn:
            ...
            count = pd.read_sql_query(
                text(f"SELECT count(*) FROM bbdd_cs_med_tuya.tb_historico_canales_autorizados"), conn)['count(*)'][0]

            conn.execute(text(
                f"delete from bbdd_cs_med_tuya.tb_historico_canales_autorizados limit {count}"))

            logging.info(f"{count} datos en la tabla preproceso")

            df.to_sql('tb_historico_canales_autorizados', con=conn,
