...
def webscraping(import_username: str, import_password: str, *_):
    ...
    try:
        ...
        asyncio.run(enviar_mensaje(f'Tuya Prejuridico Castigo'))
        ...
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, "/html/body/div[2]/div[1]/div/div[3]/div/div[2]/div[2]/div[1]/div/div[2]/div[2]/table/tbody/tr[2]/td[1]/label"))).click()
...
def transform(df: DataFrame) -> DataFrame:
    try:
        ...
        df['Fecha_Real'] = pd.to_datetime(
            df["Fecha_Real"], errors='coerce')

        df['Fecha_Ultima_Causal'] = pd.to_datetime(
            df["Fecha_Ultima_Causal"], errors='coerce')
    ...
    try:
        with engine_61.get_connect() as conn:
            ...
            count = pd.read_sql_query(
                text(f"SELECT count(*) FROM bbdd_cs_med_tuya.tb_informe_prejuridico_castigo"), conn)['count(*)'][0]

            """conn.execute(text(
                f"delete from bbdd_cs_med_tuya.tb_informe_prejuridico_castigo limit {count}"))"""

            logging.info(f"{count} datos en la tabla preproceso")

            df.to_sql('tb_informe_prejuridico_castigo', con=conn,
