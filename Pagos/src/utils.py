...
def webscraping(import_username: str, import_password: str, *_):
    ...
    try:
        ...
        asyncio.run(enviar_mensaje(f'Tuya Pagos'))
        ...
        driver.get(
            """https://172.226.124.46/vsmart/index.php?rtr=informes&ctr=InformesControlador&acc=&nom_programa=informespagos_smart&titulocriterio=Informe%20de%20Pagos%20por%20Agencia""")

        WebDriverWait(driver, 30).until(
            EC.presence_of_element_located((By.CLASS_NAME, "select2-chosen"))).click()

        WebDriverWait(driver, 30).until(
            EC.presence_of_element_located((By.XPATH, "//*[@id='select2-drop']/ul/li[14]/div"))).click()

        WebDriverWait(driver, 30).until(
            EC.presence_of_element_located((By.ID, "s2id_col"))).click()

        WebDriverWait(driver, 30).until(
            EC.presence_of_element_located((By.XPATH, "//*[@id='select2-drop']/ul/li[10]/div"))).click()
        ...
        WebDriverWait(driver, 120).until(
            EC.presence_of_element_located((By.XPATH, "/html/body/div[2]/div[1]/div/div[3]/div/div[2]/div[2]/div/div/span[1]/a[3]"))).click()

        WebDriverWait(driver, 30).until(
            EC.presence_of_element_located((By.XPATH, "/html/body/div[2]/div[1]/div/div[3]/div/div[2]/div[2]/div/div/div[2]/form/div[3]/input[1]"))).click()
...
def extract() -> DataFrame:
    try:
        ...
        asyncio.run(enviar_mensaje(f'Error tuya_pagos {e}'))
        logging.info(e)
        ...
        with engine_61.get_connect() as conn:
            ...
            count = pd.read_sql_query(
                text(f"""SELECT 
                        COUNT(*)
                        FROM
                        bbdd_cs_med_tuya.tb_informe_pagos_agencia
                        WHERE
                        YEAR(Fecha_De_Pago) = YEAR(CURDATE())
                        AND MONTH(Fecha_De_Pago) = MONTH(CURDATE());"""), conn)['COUNT(*)'][0]

            conn.execute(text(f"""DELETE
                        FROM
                        bbdd_cs_med_tuya.tb_informe_pagos_agencia
                        WHERE
                        YEAR(Fecha_De_Pago) = YEAR(CURDATE())
                        AND MONTH(Fecha_De_Pago) = MONTH(CURDATE()) LIMIT {count}"""))

            df.to_sql('tb_informe_pagos_agencia', con=conn,
